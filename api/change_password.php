...
if (strlen($new1) < 8) {
  faav_json(['ok' => false, 'error' => 'La nueva contraseña debe tener al menos 8 caracteres.'], 400);
}
if ($new1 !== $new2) {
  faav_json(['ok' => false, 'error' => 'Las contraseñas nuevas no coinciden.'], 400);
}
...
if (!$row || !password_verify($current . APP_PEPPER, $row['password_hash'])) {
  faav_json(['ok' => false, 'error' => 'La contraseña actual no es correcta.'], 401);
}
