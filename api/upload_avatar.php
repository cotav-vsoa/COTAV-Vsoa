...
if (!extension_loaded('gd')) {
  cotav_json(['ok' => false, 'error' => 'El servidor no tiene la extensión GD habilitada (necesaria para procesar imágenes). Activala desde cPanel (MultiPHP / PHP Extensions) e intentá de nuevo.'], 500);
}

if (empty($_FILES['avatar'])) {
  cotav_json(['ok' => false, 'error' => 'No se recibió ninguna imagen.'], 400);
}
...
if ($f['error'] !== UPLOAD_ERR_OK) {
  ...
  cotav_json(['ok' => false, 'error' => $msg], 400);
}

/* Límite propio, además del que ya imponga el servidor (upload_max_filesize). */
if ($f['size'] > 5 * 1024 * 1024) {
  cotav_json(['ok' => false, 'error' => 'La imagen no puede pesar más de 5 MB.'], 400);
}
...
if (!$info) {
  cotav_json(['ok' => false, 'error' => 'El archivo no es una imagen válida.'], 400);
}
...
switch ($mime) {
  ...
  case 'image/webp':
    if (!function_exists('imagecreatefromwebp')) {
      cotav_json(['ok' => false, 'error' => 'El servidor no soporta WEBP. Subí la foto en JPG o PNG.'], 400);
    }
    $src = @imagecreatefromwebp($tmp);
    break;
  default:
    cotav_json(['ok' => false, 'error' => 'Formato no soportado. Usá JPG, PNG o WEBP.'], 400);
}

if (!$src) {
  cotav_json(['ok' => false, 'error' => 'No se pudo procesar la imagen.'], 400);
}
...
if (!is_dir($dir)) {
  if (!@mkdir($dir, 0755, true) && !is_dir($dir)) {
    cotav_json(['ok' => false, 'error' => 'No se pudo crear la carpeta de destino en el servidor.', 'debug_dir' => realpath(__DIR__ . '/..') . '/img/pilotos'], 500);
  }
}
if (!is_writable($dir)) {
  cotav_json(['ok' => false, 'error' => 'La carpeta img/pilotos no tiene permisos de escritura para el servidor.', 'debug_dir' => realpath($dir)], 500);
}
...
if (!$ok || !file_exists($path)) {
  cotav_json(['ok' => false, 'error' => 'No se pudo guardar la imagen en el servidor.', 'debug_path' => $path], 500);
}
@chmod($path, 0644);
