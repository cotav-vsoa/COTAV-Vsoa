...
if($MyInvocation.InvocationName -ne '.'){ $Missing = ($PWD.Path -notlike '*FAAV'); $root = Split-Path -Parent $PSScriptRoot; Set-Location -LiteralPath $root }
...
Get-ChildItem -LiteralPath $htmlDir -Recurse -Filter *.html -File | ForEach-Object {
  ...
  $n = $c -replace ('Logo-Faav\.ico\?v=\d+'), ("Logo-Faav.ico?v=$icoNew")
...
Write-Output ("== {0} archivos actualizados (Logo-Faav.ico?v={1}, script.js?v={2}) ==" -f $changed, $icoNew, $jsNew)
